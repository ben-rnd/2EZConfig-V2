...
static unsigned char memutils_alloc(size_t amt, unsigned int is_exec, void** addr) {
    unsigned int access_mask = 0;
#ifdef TARGET_OS_WINDOWS
    if (is_exec) { access_mask = PAGE_EXECUTE_READWRITE; }
    else { access_mask = PAGE_READWRITE; }
    *addr = VirtualAlloc(NULL, MEMUTILS_PAGE_SIZE, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);
#elif TARGET_OS_LINUX || TARGET_OS_OSX
...
static unsigned char memutils_free(void* page_addr, size_t num) {
#ifdef TARGET_OS_WINDOWS
...
unsigned char memutils_get_proc(const char* lib_name, const char* func_name, void** func_address) {
    ...
    *func_address = NULL;
    ...
    if(!hl){return 0;}
    *func_address = (void*)GetProcAddress(hl, func_name);
...
unsigned char memutils_alloc_exec(void** page_addr) {
    *page_addr = NULL;
    if (!memutils_alloc(MEMUTILS_PAGE_SIZE, 1, page_addr)) { return 0; }
    return 1;
}

unsigned char memutils_free_exec(void* page_addr) {
    memset(page_addr, 0x00, MEMUTILS_PAGE_SIZE);
    if (!memutils_free(page_addr, MEMUTILS_PAGE_SIZE)) { return 0; }
    return 1;
}

static unsigned char memutils_set_permission(void* target_addr, size_t data_len, unsigned char enable_read, unsigned char enable_write, unsigned char enable_execute, unsigned int* old_prot) {
    // Default to PAGE_SIZE if no len.
    if (!data_len) { data_len = MEMUTILS_PAGE_SIZE; }
    ...
    mprotect((void*)(target_addr - ((size_t)target_addr % MEMUTILS_PAGE_SIZE)), data_len, n_prot);
...
unsigned char memutils_patch(void* target_addr, void* data_ptr, size_t data_len, unsigned char is_write, unsigned char is_exec) {
    ...
    if (!is_write) {
        if (!memutils_set_permission(target_addr, data_len, 1, 1, is_exec, &old_prot)) { return 0; }
    }

    memcpy(target_addr, data_ptr, data_len);
    if (!is_write) {
        if (!memutils_set_permission(target_addr, data_len, 1, 0, is_exec, &old_prot)) { return 0; }
    }
...
unsigned char memutils_patch_ret0(void* target_addr) {
    ...
    return memutils_patch(target_addr, x86_ret_0, sizeof(x86_ret_0), 0, 1);
}

unsigned char memutils_patch_ret1(void* target_addr) {
    unsigned char x86_ret_1[4] = { 0x31,0xC0, 0x40, 0xC3 };
    return memutils_patch(target_addr, x86_ret_1, sizeof(x86_ret_1), 0, 1);
}

unsigned char memutils_unhotpatch(struct HotPatchInfo* ctx) {
    memutils_patch(ctx->target_function_address, ctx->target_original_bytes, ctx->target_original_bytes_size, 0, 1);
    return 1;
}
...
unsigned char memutils_hotpatch(void* target_function_address, void* replacement_function_address, size_t target_original_bytes_size, struct HotPatchInfo* ctx, void** ptrampoline_address) {
    ...
    memutils_alloc(ctx->target_original_bytes_size, 0, (void**)&ctx->target_original_bytes);
    ...
    if (!memutils_alloc_exec(ptrampoline_address)) { return 0; }
    ...
    if (!memutils_patch(ctx->target_function_address, hook_stub, sizeof(hotpatch_stub), 0, 1)) { return 0; }
