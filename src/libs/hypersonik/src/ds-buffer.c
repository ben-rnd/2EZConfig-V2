...
static double g_master_scale = 256.0;

void ds_buffer_set_master_scale(double scale)
{
    g_master_scale = scale;
}
...
static HRESULT ds_buffer_apply_volume(struct ds_buffer *self)
{
    ...
    left_vol = g_master_scale * pow(10.0, left_mb / 2000.0);
    right_vol = g_master_scale * pow(10.0, right_mb / 2000.0);
